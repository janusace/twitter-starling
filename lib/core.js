...
function Twitter(login) {
	...
	this.nextLoginCredential	=	function() {
										if(this.credentials.length > 0) {
											...
											//
											// TODO:	loop through all credentials, starting at credentialIndex
											//			and see if that credential is rate limited. if it isn't,
											//			then return it as the round robin.
											//
											//			if they all are, return an error
											//
...
TwitterLogin_API = function(theparent) {
	...
	this.registerRequest = 			function(isMedia) {
									
										if(this.total_hits % 50 == 0) {	 // hard limit before checking our API status by contacting Twitter
										...
										if(this.remaining_hits <= 0) {
											...
											if(this.remaining_hits <= 0) {
												out = new TwitterResult(false, 'TwitterLogin: you have exceeded your API request allowance.');
											}
...
Twitter.prototype.request = function(url, param, authentication, callback, parent) {
	...
	this.fetch(url, param, authentication, method, callback, parent);
	
}

Twitter.prototype.fetch = function(url, param, authentication, method, callback, parent) {
	...
	if(credential.api.isRateLimited()) {	// return error indicating rate limit
		...
		request(requestParam , function(error, response, body) {
			...
			var out = null;
			if(error || !util.isUndefined(body.error) || response.statusCode != 200) {
				out = new TwitterResult( false, {
					message: body.error,
					data: body,
					code: (util.isUndefined(body.code) ? response.statusCode : body.code)
				});
			} else {
